...
*(This section describes features implemented beyond the core batch processing pipeline)*

* **Simple REST Endpoint:** A new service has been added to provide a simple REST API for real-time single address matching.
    * **Implementation:** Implemented in `src/api.py` using the Flask framework.
    * **Endpoint:** Exposes a POST endpoint `/match_address`.
    * **Functionality:** Accepts a raw address string in a JSON request body (`{"raw_address": "..."}`). It uses the parsing and normalization logic from `src/parse.py` and then applies the same matching waterfall (Exact, Fuzzy, Phonetic) implemented within `src/api.py` (adapted from the batch processing logic in `src/match.py` and `src/fallback_match.py` for single-address lookup) against the `canonical_addresses` table.
    * **Response:** Returns a JSON response containing the original raw address, parsed components, normalized address, and the best match found (including `address_id`, `match_type`, and `confidence_score`) or an `unmatch_reason` if no match was found.
    * **Integration:** Runs as a separate service within the Docker Compose setup, requiring database access.

### How to Run the API Service (using Docker Compose):

1.  **Update `docker-compose.yml`:** Add a new service block named `api` to your `docker-compose.yml` file. This service will use the same Docker image you build, expose a port, and depend on the database service.

2.  **Start the Database and API Services:** Use Docker Compose to bring up the necessary services.

    ```bash
    docker-compose up -d db api
    ```
    This command starts the `db` and `api` containers in detached mode. The `depends_on: db` in the `api` service definition ensures that the database is running before the API starts.

3.  **Initialize Data (if needed):** The API requires data to be present in the `canonical_addresses` table. If you haven't run the ingestion pipeline yet, you need to do that first. You can use the `app` service for this:

    ```bash
    # Ensure the 'app' service is running (e.g., include 'app' in the 'docker-compose up' command, or run separately)
    docker-compose up -d db api app # Example to start all three

    # Then execute the pipeline to ingest data
    docker-compose exec app python /app/run_pipeline.py
    ```
    *Running the full pipeline (`run_pipeline.py`) will ingest data, parse transactions, and perform batch matching. The API will use the data populated by this process.*

4.  **Interact with the API:** Once the `db` and `api` containers are running and you've ingested data, you can send POST requests to the API endpoint from your host machine using `curl`, Python scripts, Postman, Insomnia, or any other tool. The API is accessible on `http://localhost:5000`.

    Here's how to use `curl` from your host machine to send a request:
